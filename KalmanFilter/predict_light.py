...
def sim_kf_predict(sim_opt, KalmanFilter, light_state):
    start = time.perf_counter()
    # Simulator Options
    ALLOW_SPEEDING = sim_opt['ALLOW_SPEEDING']
    ...
    dt = 0.1
    for t0 in t:
        state += [motion(t0,dt,state,u_pedal)]
        if True:#t0%1.0 == 0.0:
            est_data_t += [t0]
            ...
            if t0 >= light_time and first_prediction:
                if not ALLOW_SPEEDING:
                    predict_x = kalman_filter.predict_light(95)
                else:
                    predict_x = kalman_filter.predict_light_speeding(95)
                if not predict_x[0]:
                    u_pedal = 0
                elif ALLOW_SPEEDING:
                    u_pedal = 6
