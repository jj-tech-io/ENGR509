...
def simulator_1d(options, KalmanFilter):
    ...
    for t0 in t:
        ...
        if t0%1.0 == 0.0:
            ...
            state_with_noise = state[-1]+(np.random.rand(1)[0]-0.5) * noise_var
            if t0 == 0.0:
                x_est_data += [0]
                v_est_data += [0]
                continue
    ...
    def update_plot(num):
        ...
        # Car.
        car_l.set_data([3, 3],[state[num], state[num]+5])
        car_r.set_data([4, 4],[state[num], state[num]+5])
        car_t.set_data([3, 4],[state[num]+5, state[num]+5])
        car_b.set_data([3, 4],[state[num], state[num]])
